...
**Comment: Framework example, sophisticated method, focus primarely on satellite data. Possibly we could take as an inspiration when starting to handle satellite images to increase the satellite data use and skin to bulk conversion and when dealing with the 3D model since kind of continues from where Baracchini left.**
