...
Every supported model implements this surface; the engines (`algorithms/enkf.py`,
`algorithms/pf.py`) and the orchestrator (`main.py`) call models *only* through it, so a
...
class Model(ABC):
    ...
    @abstractmethod
    def model_inputs_ready(self, model_inputs): ...
    @abstractmethod
    def instances_ready(self, ensemble_base, n_members): ...
    @abstractmethod
    def copy_model_inputs(self, raw): ...
    ...
    @abstractmethod
    def set_output_depths(self, model_inputs, ensemble_base, n_members, obs_depths): ...
    @abstractmethod
    def obs_to_sim_col(self, depth): ...
    @abstractmethod
    ...
    @abstractmethod
    def mean_traj_path(self, ensemble_base, algorithm): ...
    @abstractmethod
